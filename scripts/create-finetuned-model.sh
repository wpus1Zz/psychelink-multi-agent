...
MODEL_NAME="${FINETUNED_MODEL_NAME:-psychelink-qwen2.5-7b-ft:latest}"
MODEL_DIR="${FINETUNED_MODEL_DIR:-$ROOT_DIR/models/psychelink-qwen2.5-7b-ft}"
GGUF_FILE="${FINETUNED_MODEL_FILE:-psychelink-qwen2.5-7b-ft-q4_k_m.gguf}"
...
if [ ! -f "$MODEL_DIR/$GGUF_FILE" ]; then
  if [ -n "$UPSTREAM_GGUF" ] && [ -f "$UPSTREAM_GGUF" ]; then
    echo "Linking GGUF from upstream PsycheLink project..."
...
echo "Run PsycheLink Python with: AI_PROVIDER=ollama ./scripts/run-dev.sh"
