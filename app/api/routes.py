...
def agent_status(user: Annotated[UserAccount, Depends(current_user)]):
    ...
    model = (
        ...
        else settings.anthropic_model if provider == "anthropic"
        else "mock"
    )
    ...
    return {
        "provider": provider,
        "model": model,
        "realModelEnabled": provider in {"ollama", "openai", "anthropic"},
        ...
        "skills": PsycheLinkSkillLibrary.status_items(),
        "runtimeHarness": {
            "name": "PsycheLinkAgentHarness",
            "status": "READY",
