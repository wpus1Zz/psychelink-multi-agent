...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run PsycheLink engineering harness checks.")
    parser.add_argument(
        "--suite",
        action="append",
        choices=["risk", "routing", "routing-eval", "skills", "rag", "api", "tool-queue", "all"],
        default=None,
...
def configure_environment() -> None:
    ...
    db_path = target_dir / "psychelink-harness.sqlite3"
    ...
    # Keep an explicitly configured provider (for example Anthropic/DeepSeek
    # during a real-model evaluation); fall back to mock for deterministic CI.
    ...
    os.environ["EXCEL_PATH"] = str((target_dir / "psychelink-risk-ledger.xlsx").as_posix())
...
def resolve_suites(requested: list[str] | None) -> list[tuple[str, Callable[[HarnessContext], dict]]]:
    all_suites: list[tuple[str, Callable[[HarnessContext], dict]]] = [
        ...
        ("Routing Evaluation Harness", run_routing_eval_harness),
    ...
    aliases = {
        ...
        "routing-eval": "Routing Evaluation Harness",
...
def _eval_failure_details(name: str, context: HarnessContext) -> dict:
    if name == "Routing Evaluation Harness":
        report_path = context.target_dir / "routing-eval-report.json"
        metric_keys = ("totalCases", "accuracy", "riskRecall")
    elif name == "RAG Harness":
        report_path = context.target_dir / "rag-eval-report.json"
        metric_keys = ("totalCases", "hitRate", "mrr")
    else:
        return {}
    ...
    try:
        ...
        source = raw.get("metrics", {}) if name == "RAG Harness" else raw
        return {key: source.get(key) for key in metric_keys}
...
def run_check(name: str, fn: Callable[[HarnessContext], dict], context: HarnessContext) -> CheckResult:
    try:
        ...
        return CheckResult(
            name=name,
            passed=False,
            details=_eval_failure_details(name, context),
            failures=[str(exc)],
        )
...
def run_agent_routing_harness(context: HarnessContext) -> dict:
    ...
    try:
        ...
        cases = [
            {
                ...
                "must_steps": ["UnderstandingAgent", "SafetyAgent", "ResponseAgent"],
            ...
            },
            {
                ...
                "must_steps": ["UnderstandingAgent", "SafetyAgent", "ContextAgent", "ResponseAgent"],
            },
            {
                "id": "risk-safety",
        ...
        for case in cases:
            ...
            db.commit()
            db.refresh(session)
            result = PsycheLinkAgentHarness(db, context.settings).run(
                user,
...
def run_routing_eval_harness(context: HarnessContext) -> dict:
    from app.harness.routing_eval import run_routing_eval_harness as run
...
def run_standard_skills_harness(context: HarnessContext) -> dict:
    ...
    from app.services.skills import PsycheLinkSkillLibrary
    ...
    skills = PsycheLinkSkillLibrary.list_skills()
    names = {skill.name for skill in skills}
    missing = sorted(expected - names)
    expect(not missing, f"missing standard skills: {missing}")

    statuses = PsycheLinkSkillLibrary.status_items()
    ...
    selected_names = PsycheLinkSkillLibrary.response_skill_names(
        IntentType.CONSULT,
        RiskLevel.LOW,
        "我最近焦虑、失眠，考试压力也很大。",
    )
    ...
    context_text = PsycheLinkSkillLibrary.response_skill_context(
        IntentType.CONSULT,
        RiskLevel.LOW,
        "我最近焦虑、失眠，考试压力也很大。",
    )
    ...
    high_risk_names = PsycheLinkSkillLibrary.response_skill_names(
        IntentType.RISK,
        RiskLevel.HIGH,
        "我不想活了。",
    )
    ...
    handoff = PsycheLinkSkillLibrary.counselor_handoff_summary(report, user)
...
def print_report(report: dict) -> None:
    print("PsycheLink Engineering Harness")
