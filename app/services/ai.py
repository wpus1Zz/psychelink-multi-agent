...
class PromptTemplates:
    ...
    def intent_prompt(history: list[AiMessage], user_input: str) -> list[AiMessage]:
        return [
            AiMessage(role="system", content=(
                ...
                "CONSULT 包含压力、焦虑、低落、失眠、情绪倾诉、心理咨询求助；"
                "RISK 包含自杀、自残、伤人或即时危险信号。"
                "如果最近上下文已经在讨论情绪困扰、关系冲突、睡眠问题、心理咨询，"
                "当前输入又是在求助、求建议、想聊聊、想了解心理咨询服务，即使当前句子表面像普通问答，也优先判为 CONSULT。"
                "只有在明确是学习、编程、作业、工具使用、校园事务办理等普通任务时，才判为 CHAT。"
            )),
    ...
    def answer_system_prompt(intent: IntentType, risk: RiskLevel, context: str, display_name: str, skill_context: str = "") -> AiMessage:
        if intent == IntentType.CHAT:
            content = (
                "你是 PsycheLink，一个面向学生的日常陪伴与校园生活助手。"
        ...
        content = (
            "你是 PsycheLink，一个面向学生的校园心理关怀智能体。"
...
class AiClient:
    ...
    def complete(self, messages: list[AiMessage]) -> str:
        ...
        if provider == "anthropic":
            return self._anthropic(messages, stream=False)
    ...
    async def stream(self, messages: list[AiMessage]):
        ...
        if provider == "openai":
            ...
            return
        if provider == "anthropic":
            async for token in self._anthropic_stream(messages):
                yield token
            return
    ...
    def _anthropic_payload(self, messages: list[AiMessage], stream: bool) -> dict:
    ...
    def _anthropic(self, messages: list[AiMessage], stream: bool) -> str:
        payload = self._anthropic_payload(messages, stream)
        response = httpx.post(
    ...
    async def _anthropic_stream(self, messages: list[AiMessage]):
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream(
                "POST",
                f"{self.settings.anthropic_base_url.rstrip('/')}/v1/messages",
                headers=self._anthropic_headers(),
                json=self._anthropic_payload(messages, stream=True),
            ) as response:
                response.raise_for_status()
                async for line in response.aiter_lines():
                    if not line.startswith("data: "):
...
CONSULT_WORDS = ["焦虑", "抑郁", "压力", "失眠", "难过", "难受", "崩溃", "痛苦", "无助", "情绪", "心理", "咨询", "anxious", "depress", "stress"]
CONSULT_FOLLOWUP_WORDS = [
    "聊聊", "建议", "怎么办", "怎么缓解", "帮帮我", "咨询一下", "心理咨询", "可以提供什么帮助",
    "很难受", "理一理", "撑不住", "睡不着", "没有精神",
]
...
def is_support_seeking_turn(text: str) -> bool:
    normalized = text.lower()
    return any(word in normalized for word in CONSULT_FOLLOWUP_WORDS)


def history_has_consult_context(history: list[AiMessage]) -> bool:
    recent = " ".join(message.content.lower() for message in history[-8:])
    return any(word in recent for word in CONSULT_WORDS + CONSULT_FOLLOWUP_WORDS)
