...
class PsycheLinkMcpToolClient:
    ...
    async def handle_report(self, report_id: int, risk_level: str | None) -> list[str]:
        try:
            async with self._session() as session:
                results = [
                    await self._call_tool(session, "psychelink_excel_report", {"report_id": report_id}),
                ]
                case_id = None
                if risk_level in {RiskLevel.MEDIUM.value, RiskLevel.HIGH.value}:
                    case_result = await self._call_tool(session, "psychelink_case_create", {"report_id": report_id})
                    results.append(case_result)
                    case_id = self._extract_case_id(case_result)
                if risk_level == RiskLevel.HIGH.value:
                    if case_id is None:
                        raise McpToolError("高风险报告已创建，但无法解析 caseId，预警未发送")
                    results.append(await self._call_tool(session, "psychelink_alert_send", {"case_id": case_id}))
                return results
        except McpToolError:
            raise
    ...
    async def _session(self) -> AsyncIterator[Any]:
        try:
            ...
            raise McpToolError("缺少 mcp 依赖，无法通过 MCP 调用 PsycheLink 工具") from exc
