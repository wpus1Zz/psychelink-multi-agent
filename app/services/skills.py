...
class PsycheLinkSkillRegistry:
    def __init__(self, root: Path | None = None):
        self.root = root or Path(__file__).resolve().parents[2] / "skills"

    def list_skills(self) -> list[PsycheLinkSkill]:
    ...
    def get_required(self, name: str) -> PsycheLinkSkill:
    ...
    def _load_skill_file(self, path: Path) -> PsycheLinkSkill:
        ...
        return PsycheLinkSkill(name=name.strip(), description=description.strip(), body=body.strip(), path=path, metadata=metadata)


class PsycheLinkSkillLibrary:
    @staticmethod
    def registry() -> PsycheLinkSkillRegistry:
        return PsycheLinkSkillRegistry()

    @staticmethod
    def list_skills() -> list[PsycheLinkSkill]:
        return PsycheLinkSkillLibrary.registry().list_skills()

    @staticmethod
    def status_items() -> list[dict]:
        return PsycheLinkSkillLibrary.registry().status_items()

    @staticmethod
    def response_skill_context(intent: IntentType, risk: RiskLevel, text: str) -> str:
        names = PsycheLinkSkillLibrary.response_skill_names(intent, risk, text)
        registry = PsycheLinkSkillLibrary.registry()
    ...
    def high_risk_safety_plan_prompt() -> str:
        return PsycheLinkSkillLibrary.registry().get_required("high_risk_safety_plan").prompt_context()

    @staticmethod
    def counselor_handoff_summary(report: PsychologicalReport, user: UserAccount | None) -> str:
        template = PsycheLinkSkillLibrary.registry().template_for("counselor_handoff_summary")
