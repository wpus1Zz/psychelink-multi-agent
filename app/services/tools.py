...
class ToolOrchestrationService:
    ...
    def write_excel(self, report: PsychologicalReport) -> ExcelRecord:
        ...
        with EXCEL_WRITE_LOCK:
            if path.exists():
                ...
                workbook = Workbook()
                sheet = workbook.active
                sheet.title = "PsycheLink Risk Ledger"
    ...
    def create_case(self, report: PsychologicalReport) -> RiskCase:
        ...
        case = RiskCase(
            ...
            handoff_summary=PsycheLinkSkillLibrary.counselor_handoff_summary(report, user),
        )
        self.db.add(case)
        self.db.commit()
        return case
    ...
    def _email_body(self, report: PsychologicalReport, case: RiskCase | None = None) -> str:
        ...
        handoff = case.handoff_summary if case else PsycheLinkSkillLibrary.counselor_handoff_summary(report, user)
        return "\n".join(
            [
                "PsycheLink 检测到一条高风险心理预警，请尽快安排辅导员或管理员跟进。",
                "",
