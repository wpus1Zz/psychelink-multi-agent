...
mcp = FastMCP("psychelink-python-tools")


@mcp.tool()
def psychelink_excel_report(report_id: int) -> str:
    """Write one psychological risk report into the PsycheLink Excel ledger."""
    create_schema()
    db = SessionLocal()
    try:
...
@mcp.tool()
def psychelink_case_create(report_id: int) -> str:
    """Create or return the active PsycheLink risk case for one psychological report."""
    create_schema()
    db = SessionLocal()
    try:
        report = db.get(PsychologicalReport, report_id)
...
@mcp.tool()
def psychelink_alert_send(case_id: int) -> str:
    """Send or record the counselor alert for one PsycheLink risk case."""
    create_schema()
    db = SessionLocal()
    try:
...
@mcp.tool()
def psychelink_alert_ack(case_id: int, actor: str, note: str = "") -> str:
    """Mark a PsycheLink risk case as acknowledged by a counselor or administrator."""
    create_schema()
    db = SessionLocal()
    try:
...
@mcp.tool()
def psychelink_case_note_add(case_id: int, actor: str, note: str) -> str:
    """Append a follow-up note to a PsycheLink risk case."""
    create_schema()
    db = SessionLocal()
    try:
...
@mcp.tool()
def psychelink_alert_notify(report_id: int) -> str:
