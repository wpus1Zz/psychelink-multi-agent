...
from app.services.mcp_client import PsycheLinkMcpToolClient
...
class PsycheLinkAgentHarness:
    """Runtime harness for one PsycheLink agent turn.
    ...
    async def dispatch_tools(self, tool_plan: AgentToolPlan) -> list[str]:
        ...
        if self.settings.tool_queue_enabled:
            ...
            return ["queued"]
        return await PsycheLinkMcpToolClient(self.settings).handle_report(tool_plan.report_id, tool_plan.risk_level)
