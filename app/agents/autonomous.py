...
class UnderstandingAgent(BaseAutonomousAgent):
    ...
    def _classify(self, text: str, board: CollaborationBlackboard) -> IntentType:
        ...
        if has_consult_signal(lowered) or is_support_seeking_turn(lowered):
        ...
        try:
            ...
            pass
        return IntentType.CONSULT if (has_consult_signal(lowered) or is_support_seeking_turn(lowered)) else IntentType.CHAT
...
class ContextAgent(BaseAutonomousAgent):
    ...
    def act(self, task: AgentTask, board: CollaborationBlackboard) -> AgentTurnResult:
        ...
        if intent != IntentType.CHAT or risk != RiskLevel.LOW:
            ...
            skill_context = PsycheLinkSkillLibrary.response_skill_context(intent, risk, board.user_input)
        payload = {
