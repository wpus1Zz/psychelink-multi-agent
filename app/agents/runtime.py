...
from app.services.ai import (
    ...
    history_has_consult_context,
    is_support_seeking_turn,
)
...
class AgentRuntimeService:
    ...
    def counselor_agent(self, step: int, context: AgentContext) -> bool:
        ...
        skill_context = PsycheLinkSkillLibrary.response_skill_context(
    ...
    def _classify(self, text: str, history: list[AiMessage]) -> IntentType:
        ...
        if is_support_seeking_turn(lowered) and history_has_consult_context(history):
            return IntentType.CONSULT
        ...
        try:
            ...
            pass
        if history_has_consult_context(history) and is_support_seeking_turn(lowered):
            return IntentType.CONSULT
        return IntentType.CHAT
    ...
    def _rewrite_query(self, context: AgentContext) -> str:
        try:
            query = self.ai.complete([
                AiMessage(role="system", content="你是 PsycheLink 的 KnowledgeAgent。把学生输入改写成适合检索校园心理知识库的中文查询词，只输出查询词。"),
    ...
    def _summarize_memory(self, history: list[AiMessage], current_input: str, fallback: str) -> str:
        ...
        try:
            summary = self.ai.complete([
                AiMessage(role="system", content="你是 PsycheLink 的 MemoryAgent。只输出 1-3 条中文记忆要点，不输出风险等级或诊断。"),
